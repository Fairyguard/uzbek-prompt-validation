...
export async function recomputePromptState(
  ...
  const canonicalDecision = chooseCanonicalUzbek(prompt.reviews, prompt.canonicalUzbekPrompt);
  const intentMatchUpdates = prompt.intentChecks.filter((intentCheck) => {
  ...
  const intentSummary = buildIntentSummary(
    refreshedPrompt.intendedIntent,
    refreshedPrompt.intentChecks,
  );
  const state = computePromptState({
    prompt: {
      ...
      openReviewAssignments: refreshedPrompt.assignments.filter(
        (assignment) =>
      ...
      openIntentAssignments: refreshedPrompt.assignments.filter(
        (assignment) =>
  ...
  await tx.prompt.update({
    ...
    data: {
      ...
      disagreementFlag: reviewSummary.disagreement || intentSummary.disagreement,
      escalationReasons: JSON.stringify(state.escalationReasons),
      reviewSummary: JSON.stringify(reviewSummary),
      intentCheckSummary: JSON.stringify(intentSummary),
      lastReviewedAt: refreshedPrompt.reviews.at(-1)?.createdAt ?? null,
      lastIntentCheckedAt: refreshedPrompt.intentChecks.at(-1)?.createdAt ?? null,
      finalizedAt: state.finalDecision ? new Date() : null,
      extraReviewRequested:
        nextStatus === PromptStatus.PENDING_INTENT_CHECK ||
        nextStatus === PromptStatus.IN_INTENT_CHECK ||
        nextStatus === PromptStatus.APPROVED ||
        nextStatus === PromptStatus.NEEDS_REVISION
          ? false
...
export async function assignPromptToUser(input: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (input.taskType === TaskType.REVIEW && reviewCount >= reviewTarget && !overrideLimit) {
    ...
    if (
      ...
      (intentCount >= prompt.requiredIntentChecks ||
        ...
        prompt.needsCanonicalSelection) &&
      !overrideLimit
    ) {
      throw new Error("This prompt is not eligible for intent checking.");
    }

    if (input.taskType === TaskType.SPOT_CHECK && prompt.status !== PromptStatus.PENDING_SPOT_CHECK) {
...
export async function autoAssignPrompts(input: {
  ...
  const candidates = prompts.filter((prompt) => {
    ...
    if (
      input.taskType === TaskType.REVIEW &&
      prompt.reviews.length >= reviewTarget &&
      !input.overrideLimit
    ) {
      return false;
    }
  ...
  const sorted = [...candidates].sort((left, right) => {
    ...
    const rightTouched = right.assignments.some((assignment) => assignment.userId === input.userId);
